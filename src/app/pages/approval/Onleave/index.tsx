...
import { content, contentApproval, dataHeader, dataHeaderApproval } from "@/app/data/onleave";
...
import { addDataApprovalOnleave, approveAllRequestOnleave, approveOnleaveById, approveRequestOnleave, getDataApprovalOnleaveDetail, getDataApprovalOnleaveRemaining, getDataApproverOnleavePagination, getDataOnleaveOnleavePagination, rejectAllRequestOnleave, rejectOnleaveById } from "@/app/services/approval/onleave";
...
const Onleave = () => {
  ...
  const getDatasApprovalPagination = async (params) => {
    setIsLoading(true);
    try {
      const response = await getDataOnleaveOnleavePagination(params) as any;
  ...
  const getDatasApproverPagination = async (params) => {
    setIsLoading(true);
    try {
      const response = await getDataApproverOnleavePagination(params) as any;
      if (response.data.length > 0) {
  ...
  useEffect(() => {
    if(dateStart) {
      const nextDate =  moment(dateStart, "DD-MM-YYYY").add(remainingLeave, 'days');
      setDateRemaining(nextDate)
    }
  }, [dateStart])
  ...
  const handleSaveRequest = async () => {
    ...
    if(!isleaveMoreThanBalance && (totalDate > remainingLeave)) {
      snackBar('error', 'Leave Can`t be extended')
    } else {
        const payload = {
          leaveName: overtimeType,
          document: document,
      ...
      const response = await addDataApprovalOnleave(payload)
      if(response){
        const params = {
          search: isEmpty(query) ? null : query,
        }
  ...
  const handleApproveStatus = async (isApprove: boolean) => {
    ...
    if(showModalApproval.approvalAll) {
      const response = await approveAllRequestOnleave(noteApprove);
    } else if(showModalApproval.rejectAll) {
      const response = await rejectAllRequestOnleave(noteApprove);
    } else if (showModalApproval.approvalSelect) {

      const response = await approveOnleaveById(payloadSelect, noteApprove)

    } else if (showModalApproval.rejectSelect) {
      const response = await rejectOnleaveById(payloadSelect, noteApprove)
      ...
      const response = await approveRequestOnleave(showModalApproval.id, payload);
    }
  ...
  const handleApproveById = async (id: string) => {
    const response = await getDataApprovalOnleaveDetail(id);
    if(response) {
  ...
  const handleGetRemaining = async() => {
    const response = await getDataApprovalOnleaveRemaining(overtimeType);
  ...
  const handleGetDataLeave = async () => {
    const response = await getDataLeavePagination({});

    let dataLeaves:Array<object> = [{}];

    response.data.map(data => dataLeaves.push({
      label: data.leaveName,
      value: data.leaveCode,
      leaveMoreThanBalance: data.leaveMoreThanBalance
    }))
                          ...
                          <tr key={key}>
                            ...
                            <td className={className}>
                                ...
                                >
                                  ...
                                  {`${data ? data?.fromDate?.slice(0, 10) : ''} ${data.fromDate && data.fromDate?.slice(11, 16)}`}
                                </Typography>
                            </td>
                            <td className={className}>
                                <Typography
                                ...
                                className="font-semibold"
                                >
                                  {`${data ? data.toDate.slice(0, 10) : ''} ${data ? data.toDate.slice(11, 16) : '-'}`}
                                </Typography>
                            </td>
                            <td className={className}>
                              <Typography
                          ...
                          return (
                            <tr key={key}>
                              ...
                              <td className={className}>
                                  ...
                                  className="font-semibold"
                                  >
                                    {`${data ? data?.fromDate?.slice(0, 10) : '-'} ${data ? data?.fromDate?.slice(11, 16) : '-'}`}
                                  </Typography>
                              </td>
                              <td className={className}>
                                  <Typography
                                    ...
                                    className="font-semibold"
                                  >
                                    {`${data ? data.toDate.slice(0, 10) : '-'} ${data ? data?.fromDate?.slice(11, 16) : '-'}`}
                                  </Typography>
                              </td>
          ...
          body={
            <div className="w-full grid gap-6 md:grid-cols-2 xl:grid-cols-2">
              <Select
                ...
                placeholder={<Typography variant="small" className="text-blue-gray-500 font-normal">Onleave Type *</Typography>}
                isClearable
                options={dataLeave ?? undefined}
                onChange={(selected: any) => {
                  setOvertimeType(selected.label);
                  setIsLeaveMoreThanBalance(selected.leaveMoreThanBalance)
                }}
                value={overtimeType ? {label: overtimeType, value: overtimeType}: null}

              />

              <Input size="md" value={remainingLeave === 0 ? '' : `${remainingLeave} Days Remaining`} disabled label="Total Leave Remaining" color="green"/>
              <InputDatePicker withTime size={'md'} label="Start Onleave *" value={dateStart} onChange={(e: any) => setDateStart(e)} />
              <InputDatePicker withTime size={'md'} label="End Onleave *" value={dateEnd} onChange={(e: any) => setDateEnd(e)} />
